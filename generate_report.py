...
class SecurityReportAnalyzer:
        ...
        try:
            with open(file_path, "r") as f:
                ...
                if content.startswith("Running kube-bench on node:"):
                    ...
                    node_info = node_line.replace(
                        "Running kube-bench on node:", ""
                    ).strip()
    ...
    def analyze_kubescape(self, kubescape_data: dict) -> Dict[str, Any]:
        ...
        findings = {
            ...
            "skipped_resources": resource_counters.get("skippedResources")
            + resource_counters.get("excludedResources"),
            "controls": [],
            "failed_resources": [],
            "controls_stats": {},
        }
        ...
        controls_stats = {"Critical": [], "High": [], "Medium": [], "Low": []}
        controls_severity = {}
                ...
                controls_stats[severity].append(
                    {
                        ...
                        "failed_resources": control.get("ResourceCounters").get(
                            "failedResources"
                        ),
                        "all_resources": control.get("ResourceCounters").get(
                            "failedResources"
                        )
                        + control.get("ResourceCounters").get("passedResources"),
                    }
                )
                    ...
                    for rule in control_rules:
                        if rule.get("status") == "failed":
                            for path in rule.get("paths", []):
                                remediation = path.get(
                                    "failedPath",
                                    path.get("deletePath", path.get("reviewPath")),
                                )
                                if not remediation:
    ...
    def compute_markdown_diff_from_strings(
    ...
    def generate_report(
        self, kubebench_path: str, kubescape_path: str, previous_report_content=None
    ) -> tuple[str, str]:
        # Load both reports
        ...
        kubebench_total_checks = (
            kubebench_totals["total_pass"]
            + kubebench_totals["total_fail"]
            + kubebench_totals["total_warn"]
            + kubebench_totals["total_info"]
        )
        kubebench_fail_percentage = round(
            kubebench_totals["total_fail"] / kubebench_total_checks * 100, 2
        )
        ...
        risk_level = self.calculate_risk_level(
            total_fails, kubebench_totals.get("total_warn")
        )
...
"""
                        ...
                        for result in test["results"]:
                            if result["status"] in ["FAIL", "WARN"]:
                                remediation_text = self.format_remediation(
                                    result["remediation"]
                                )
                                        ...
                                        if len(remediation_lines) > 1:
                                            remediation_md += "\n" + "\n".join(
                                                [
                                                    ...
                                                    for line in remediation_lines[1:]
                                                ]
                                            )
                                    else:
                                        remediation_md = ""
...
- **Test {result['test_number']}**: {result['test_desc']}
  ...
  - **Reason**: <br>{result.get('reason', "Expected result (" + result['expected_result'] + ") not found")}
...
"""
            ...
            for control in resource["controls"]:
                docs_link = (
                    f"[C-{control['control_id'].split('-')[-1]}](https://hub.armosec.io/docs/{control['control_id']})"
...
"""
        ...
        if previous_report_content:
            ...
            raw_diff = self.compute_markdown_diff_from_strings(
                previous_report_content, report_body
            )
            if raw_diff:
                report_body += (
...
def main():
    ...
    report, diff_summary = analyzer.generate_report("kubebench.json", "kubescape.json")
